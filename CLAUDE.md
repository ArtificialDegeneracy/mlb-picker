...
## Recently fixed
- 2026-07-12 — Goose projected O/U reworked to Vegas-anchored (closes the audit finding below). Regressing (actual − vegas_total) on the FIP/xwOBA/bullpen/offense components found zero signal (R² 0.007, all |t| < 1.1, n=260 clean night games 6/13-7/11), so refitting component weights was rejected as overfit. New form in `output/goose_dashboard.py:projected_total`: `goose = vegas_total + clip(0.30 × component_lean, ±0.5)` — calibrated to the line by construction, expresses only a ≤half-run lean (MAE 3.69 vs old 3.75, Vegas 3.66). Anchors outside [6.0, 14.0] are treated as day-game-contaminated and fall back to `9.0 × park + lean` (old `_TOTAL_BASE=8.5` was below the ~9.2 run environment; 9.0 fitted on the same night-game set). Still display-only.
- 2026-07-12 — **Platoon-splits NULL root cause: the FanGraphs refresh never ran.** `refresh_fangraphs_stats` gated freshness on `MAX(team_stats.updated_at)`, but the team-records write in `refresh_data` bumps `updated_at` for all 30 teams immediately before it — so the check read "fresh < 7 days" on every run and the FanGraphs pull was silently skipped since ~early season. The API fetch/parse were never broken (the 5/20 retry fix targeted the wrong layer); wrc_plus/bullpen_era only *looked* alive because stale values persisted. Also why NYY/SF bullpen 0.0 never self-repaired. Fix: freshness tracked in a dedicated `fangraphs_refresh_log` table (season → refreshed_at), written only after a successful FanGraphs update; window tightened 7 days → 12 hours so the morning cron refreshes daily and stuck rows self-heal via the plausibility filter. Seed DB migrated + force-refreshed: 30/30 platoon splits populated, NYY/SF bullpen repaired, stale overall wRC+ corrected (e.g. BOS 102.9 → 92.9).
- 2026-07-12 — Full-system audit + retrain (data through 7/11). Findings:
  ...
  - **bdl_odds_today day-game contamination**: lineup_lock crons re-ingest odds up to ~5-6 PM ET with last-write-wins, so day games store in-game/settled odds (stored favorites won 90.1% with 85.5% mean implied prob on 131 day games vs 58.9%/56.7% on night games). Edge Meter is garbage for day games; filter any odds backtest to night games until fixed. **Ingest fixed 2026-07-12**: `data/balldontlie.py:_ingest_games_and_odds` now skips the odds upsert once a game's scheduled start (bdl `date` timestamp, UTC) has passed, keeping the last pre-game snapshot (`updated_at` = snapshot time). Rows written BEFORE the fix deployed remain contaminated — backtests spanning earlier dates must still filter day games out.
  ...
  - **Goose projected O/U has no signal**: MAE 3.77 vs Vegas 3.65 vs constant-9.2 3.72 on clean games; betting its lean vs the line is ~50%. ~~Display flavor only until recalibrated~~ — reworked to Vegas-anchored later same day, see entry above.
  - **Platoon wRC+ regression**: all 30 teams NULL for 2026 (broken since before 6/13 despite the 5/20 fix). NYY/SF `bullpen_era=0.0` was stuck again — NULLed in the deployed DB (falls back to 2025); the fangraphs plausibility filter blocks bad writes but never repairs stuck rows, and `features._get_bullpen_era` has no <=1.0 guard. **Root-caused and FIXED later on 7/12 — see entry above: the refresh was never running at all.**
